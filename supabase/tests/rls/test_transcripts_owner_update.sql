-- SEC-06: signed-in users can UPDATE their own transcripts; cannot UPDATE another user's.
...
-- Act as user A: update own row — expect 1 row affected.
...
begin
  update public.transcripts set title = 'A updated' where id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
  get diagnostics rows_affected = row_count;
  if rows_affected <> 1 then
    raise exception 'SEC-06 violated: user A own-row UPDATE affected % rows, expected 1', rows_affected;
  end if;
end $$;

-- User A tries to update user B's row — expect 0 rows affected (RLS hides it).
do $$
declare rows_affected int;
begin
  update public.transcripts set title = 'hacked' where id = 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb';
  get diagnostics rows_affected = row_count;
  if rows_affected <> 0 then
    raise exception 'SEC-06 violated: user A cross-user UPDATE affected % rows, expected 0', rows_affected;
  end if;
end $$;
